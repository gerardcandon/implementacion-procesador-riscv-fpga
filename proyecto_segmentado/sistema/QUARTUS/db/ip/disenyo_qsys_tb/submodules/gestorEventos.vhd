...
architecture compor of gestorEventos is

begin

int_y_excep: process(ultimo_ciclo, excep, excep_SYS, irq, excep_causa, excep_causa_SYS, mip, mstatus)
...
begin
	...
	--interrupciones
	if (mstatus(MIE) = '1' and ultimo_ciclo = '1') then
		if (mip(MTIP) = '1') then --reloj
			v_int_CSR := '1';	
			v_int_causa_CSR := causa_int_clk;
		elsif (irq(0) = '1') then --teclado PS/2
			v_int_CSR := '1';
			v_int_causa_CSR := causa_int_PS2;
		end if;
		
	--excepciones
	elsif (excep = '1' or excep_SYS = '1') then
		v_excep_CSR := '1';
