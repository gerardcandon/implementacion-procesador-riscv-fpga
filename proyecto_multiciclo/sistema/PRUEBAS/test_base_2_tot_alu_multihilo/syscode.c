...
void cambio_thread() {
	...
	unsigned int *oldTCB_CP;
	unsigned int *newTCB_CP;
	if (*curThr == 0) {
		*curThr = 1;
		oldTCB_CP = (unsigned int *)TCB0_CP;
		newTCB_CP = (unsigned int *)TCB1_CP;
	}
	else {
		*curThr = 0;
		oldTCB_CP = (unsigned int *)TCB1_CP;
		newTCB_CP = (unsigned int *)TCB0_CP;
	}
