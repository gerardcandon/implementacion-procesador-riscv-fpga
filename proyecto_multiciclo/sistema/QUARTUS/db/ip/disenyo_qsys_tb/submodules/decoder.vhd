...
--registros mapeados en memoria
signal read_csr_s, write_csr_s: std_logic;
...
camino: decocamino port map (estado => estado, instr => instr, 
							...
							bit_0 => bit_0_s,	mdato_sel => mdato_sel_s, mRLDM_sel => mRLDM_sel_s);
...
MEMORIA: decoMem port map (estado => estado, instr => instr, byteenable => byteenable_s, 
									...
									read_csr => read_csr_s, write_csr => write_csr_s);
...
peCP <= peCP_s and not excep_s after ret_decoder;
peCPinst <= peCPinst_s and not excep_s after ret_decoder;
mdir_sel <= mdir_sel_s after ret_decoder;
mRLDM_sel <= mRLDM_sel_s after ret_decoder;
PERI <= PERI_s after ret_decoder;
mEBR_sel <= mEBR_sel_s after ret_decoder;
PBR <= PBR_s and not excep_s after ret_decoder;
...
write <= write_s and not excep_s after ret_decoder;
read_csr <= read_csr_s after ret_decoder;
write_csr <= write_csr_s after ret_decoder;
